...
bool seedDictionaryData(QSqlDatabase &db)
{
    ...
    if (!execSchemaQuery(query,
        ...
        || !execSchemaQuery(query,
                            QString("%1 INTO types(id,name) VALUES('%2','Monitor')").arg(insertPrefix, t2),
                            "Błąd dodawania typu Monitor:")
        || !execSchemaQuery(query,
                            QString("%1 INTO types(id,name) VALUES('%2','Kabel')").arg(insertPrefix, t3),
                            "Błąd dodawania typu Kabel:")) {
        return false;
    }
    ...
    if (!execSchemaQuery(query,
        ...
        || !execSchemaQuery(query,
                            QString("%1 INTO vendors(id,name) VALUES('%2','Commodore')").arg(insertPrefix, v2),
                            "Błąd dodawania producenta Commodore:")
        || !execSchemaQuery(query,
                            QString("%1 INTO vendors(id,name) VALUES('%2','Sinclair')").arg(insertPrefix, v3),
    ...
    if (!execSchemaQuery(query,
        ...
        || !execSchemaQuery(query,
                            QString("%1 INTO statuses(id,name) VALUES('%2','W naprawie')").arg(insertPrefix, s3),
